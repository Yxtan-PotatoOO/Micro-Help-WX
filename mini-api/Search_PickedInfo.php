...
$sql = "SELECT a.id,a.usrID,a.sendDateTime,a.status,a.imgUrl1,a.imgUrl2,a.pickedAddress,a.pickedTime,a.pickedRemarks,a.phone,a.contactID,a.comTime,b.grade,b.nickName,b.avatarUrl FROM tb_PickedInfo a, tb_users b WHERE a.usrID = b.openID and a.status=0 and a.pickedRemarks LIKE '%$keyword%' order by case when a.status=0 then 1 else 0 end DESC, a.sendDateTime DESC";

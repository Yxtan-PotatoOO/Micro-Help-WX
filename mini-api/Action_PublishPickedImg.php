...
if(is_uploaded_file($_FILES['file']['tmp_name'])) {  
    ...
    $upToPath = $_SERVER['DOCUMENT_ROOT']."/images/Picked";  
    ...
    if(move_uploaded_file($tmpFile,iconv("utf-8","gb2312",$move_to_file))) {
		$check_sql = "SELECT imgUrl1 FROM tb_pickedinfo WHERE id=$id";
		$check_result = mysqli_query($conn,$check_sql);
		$check_array = mysqli_fetch_array($check_result);
		
		$check_sql2 = "SELECT imgUrl2 FROM tb_pickedinfo WHERE id=$id";
		$check_result2 = mysqli_query($conn,$check_sql2);
		$check_array2 = mysqli_fetch_array($check_result2);
		
		if($check_array['imgUrl1'] == NULL)
		{
			$sql = "UPDATE tb_pickedinfo SET imgUrl1 = '$saveName' WHERE id=$id";
			if($conn->query($sql))
			{
				echo "成功";
			}
		...
		{
			$sql = "UPDATE tb_pickedinfo SET imgUrl2 = '$saveName' WHERE id=$id";
			if($conn->query($sql))
			{
				echo "成功";
			}
		...
		{
			$sql = "UPDATE tb_pickedinfo SET imgUrl3 = '$saveName' WHERE id=$id";
			if($conn->query($sql))
			{
				echo "成功";
			}
        ...
        $sql = "DELETE tb_pickedinfo WHERE id=$id";
		$conn->query($sql);
		echo "失败";
    }  
} else {  
